<?php
	...
	class Modele {
		...
		public function selectWhereReservation($user_email)
		{
			if($this->unPDO != null){
			$requete ="select * from reservation where email = :user_email; ";
			$donnees = array(":user_email"=>$user_email); 
			$select = $this->unPDO->prepare($requete); 
			$select->execute($donnees);
			$lesReservations = $select->fetchAll();
